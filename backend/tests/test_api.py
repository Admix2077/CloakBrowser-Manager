...
def test_export_profile_bundle_local_storage_rejects_malformed_ipv6_origins_without_reading_page(
    app_client: TestClient,
):
    create = app_client.post("/api/profiles", json={"name": "Malformed IPv6 Bundle LocalStorage"})
    pid = create.json()["id"]
    page = _automation_page(url="https://[2001:db8::1/dashboard?token=hidden#frag")
    _automation_running_profile(pid, pages=[page])
    ...
    assert resp.json() == {"detail": "Local storage origin unavailable"}
    page.evaluate.assert_not_called()
    assert "2001:db8" not in resp.text
    assert "token=hidden" not in resp.text
    assert "#frag" not in resp.text
    assert _audit_events_except("profile.created") == []
    main.browser_mgr.running.pop(pid, None)
...
def test_automation_url_evidence_redacts_ip_literal_hosts(app_client: TestClient):
    create = app_client.post("/api/profiles", json={"name": "AutomationIpHostUrlRedaction"})
    pid = create.json()["id"]
    page = _automation_page(
        "https://203.0.113.45/app?viewer_token=page-secret#frag",
        "IP host",
    )
    page.automation_console_logs = [
        {
            ...
            "text": "ready",
            "location": {
                "url": "https://198.51.100.20:8443/static/app.js?token=console-secret#frag",
                "lineNumber": 1,
            },
        }
    ]
    page.automation_network_events = [
        {
            "event": "request",
            "method": "GET",
            "url": "https://[2001:db8::45]:9443/api?token=network-secret#frag",
            "resource_type": "xhr",
        }
    ]
    _automation_running_profile(pid, [page])
...
def test_automation_url_evidence_redacts_malformed_ipv6_urls(app_client: TestClient):
    create = app_client.post("/api/profiles", json={"name": "AutomationMalformedIpv6UrlRedaction"})
    pid = create.json()["id"]
    page = _automation_page(
        "https://[2001:db8::1/app?viewer_token=page-secret#frag",
        "Malformed IPv6",
    )
    page.automation_console_logs = [
        {
            ...
            "text": "ready",
            "location": {
                "url": "https://[2001:db8::2/static/app.js?token=console-secret#frag",
                "lineNumber": 1,
            },
        }
    ]
    page.automation_network_events = [
        {
            "event": "request",
            "method": "GET",
            "url": "https://[2001:db8::3/api?token=network-secret#frag",
            "resource_type": "xhr",
        }
    ]
    _automation_running_profile(pid, [page])
