...
当前 `POST /api/tasks/{id}/run` 已实现第一版 `open_url`、`click`、`fill`、`scroll` 和 `wait` step。后续接入 page 级 step 时继续复用现有 Automation REST helper，但不得把 URL query/fragment、`click.selector`、`fill.selector/value`、`keyboard_type.text`、`evaluate.expression/result`、screenshot 内容、clipboard 内容、console text、network URL/query/header/body 直接写入 task result/log。
