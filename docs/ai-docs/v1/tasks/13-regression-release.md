...
## 2026-06-07 Automation about URL marker release guardrail

背景：

- Release convergence 继续检查 Automation pages API evidence，因为 pages summary 和页面动作响应会把页面 URL 返回给 Manager UI/调用方。
- 旧 `_automation_public_about_url()` 会剥离 `about:` URL 的 query/fragment，但不会识别 path 自身的 marker；`about:runtime_service_token...` 会作为低敏 URL evidence 回显。
- 当前策略下，Pixelscan `PXLSCN-FINGERPRINT-MASKING`、IPhey 以及同类底层/第三方 fingerprint 检测失败先标阻塞，不在 Manager 侧硬解；本轮只收 Manager 自己可控的 Automation pages `about:` URL evidence 边界。

已覆盖：

- Automation pages response 的 `about:` URL path 现在复用 sensitive marker 检测。
- 含 token/header/runtime-service marker 的 `about:` path 折叠为空字符串，不回显 marker、query token 或 fragment。
- 正常 `about:blank`、internal about page filtering、http/https URL redaction、title redaction、console/network summary URL redaction 和 Automation action semantics 保持不变。

验证：

```bash
.venv/bin/python -m pytest backend/tests/test_api.py::test_automation_pages_redacts_about_url_sensitive_marker_path -q
# RED then GREEN；旧 response 回显 about:runtime_service_token_automation_about_marker；GREEN 1 passed
...
## 2026-06-07 Runtime external session token marker release guardrail

背景：

- Release convergence 继续检查 runtime session response evidence，因为 `/api/runtime/sessions/{id}` 会把 `external_session_id` 返回给 Manager 调用方。
- 旧敏感 external session id 过滤规则已覆盖 URL、credentials、runtime service token、API key 和 session id marker，但没有覆盖 marker-only 的 `auth_token...` / `auth-token...` 与 `viewer_token...` / `viewer-token...`。
- 当前策略下，Pixelscan `PXLSCN-FINGERPRINT-MASKING`、IPhey 以及同类底层/第三方 fingerprint 检测失败先标阻塞，不在 Manager 侧硬解；本轮只收 Manager 自己可控的 runtime session response evidence 边界。

已覆盖：

- Runtime session response 现在把 marker-only auth/viewer token external session id 折叠为 `unknown`。
- 公开业务 session id 仍保持可见，例如 `pm-session-token`，避免误伤正常 Project Mileage 风格的 session identifier。
- Runtime session persistence、viewer token schema、VNC forwarding、Automation worker lease/task execution 和 browser runtime 行为保持不变。
...
# RED then GREEN；旧 response 回显 auth_token-runtime-external-marker；GREEN 1 passed
...
- 这是 Manager-controlled runtime session response evidence 防御，不是 Pixelscan/IPhey 或 `PXLSCN-FINGERPRINT-MASKING` 修复。
- 同类底层/第三方 fingerprint 检测站失败先标阻塞项，再继续推进 Manager 可控 API/UI/log/audit/diagnostics/runtime/proxy/profile evidence。
- 不改变 browser runtime、`invisible_playwright` 包、stealth prefs、fingerprint seed、WebGL/canvas/noise、UA、locale/timezone、WebRTC、proxy resolution、VNC forwarding、runtime session storage、viewer token schema、Automation worker lease behavior 或 browser fingerprint 行为。
...
## 2026-06-07 CSV import auth/viewer proxy source marker release guardrail

背景：

- Release convergence 继续检查 CSV import evidence，因为 preview/import response 会把 proxy source 以脱敏形态返回给 Manager UI/调用方。
- 旧 proxy source marker 检查已覆盖 `auth_token` 与 `viewer_token`，但没有覆盖 hyphen 形态 `auth-token` 与 `viewer-token`；proxy URL redaction 后仍可能回显 marker host。
- 当前策略下，Pixelscan `PXLSCN-FINGERPRINT-MASKING`、IPhey 以及同类底层/第三方 fingerprint 检测失败先标阻塞，不在 Manager 侧硬解；本轮只收 Manager 自己可控的 CSV import proxy source evidence 边界。

已覆盖：

- CSV import preview 和 confirmed import 的 `source.proxy` 在 proxy URL redaction 后检查 auth/viewer token marker host。
- 命中 `auth-token`、`auth_token`、`viewer-token`、`viewer_token` 的 proxy source 折叠为 `[redacted]`。
- 不改变导入行验证、profile 创建、proxy normalization、audit metadata、runtime browser 或 proxy resolution 行为。
...
# RED then GREEN；旧 source.proxy 回显 http://auth-token-csv-proxy-marker.example:8080；GREEN 1 passed
...
- 同类底层/第三方 fingerprint 检测站失败先标阻塞项，再继续推进 Manager 可控 API/UI/log/audit/diagnostics/runtime/proxy/profile evidence。
- 不改变 browser runtime、`invisible_playwright` 包、stealth prefs、fingerprint seed、WebGL/canvas/noise、UA、locale/timezone、WebRTC、proxy resolution、VNC forwarding、runtime session storage、viewer token schema、Automation worker lease behavior 或 browser fingerprint 行为。
- `cbim-23h.1` 的最终 all-clear 仍不能声称 Pixelscan/IPhey 全通过；本轮只收 CSV import auth/viewer proxy source marker release boundary。

## 2026-06-07 CSV import hyphen token proxy source marker release guardrail

背景：

- Release convergence 继续检查 CSV import proxy source evidence，因为 source proxy redaction 依赖 marker list，容易和其他 sanitizer 发生列表漂移。
- 旧检查已覆盖 underscore token marker，但漏了 `access-token`、`refresh-token`、`runtime-service-token`、`service-token` host marker。
- 当前策略下，Pixelscan `PXLSCN-FINGERPRINT-MASKING`、IPhey 以及同类底层/第三方 fingerprint 检测失败先标阻塞，不在 Manager 侧硬解；本轮只收 Manager 自己可控的 CSV import proxy source evidence 边界。

已覆盖：

- CSV import preview 和 confirmed import 的 `source.proxy` 会把剩余 hyphen token proxy host marker 折叠为 `[redacted]`。
- 不改变导入行验证、profile 创建、proxy normalization、audit metadata、runtime browser 或 proxy resolution 行为。
...
## 2026-06-07 CSV import source IP literal release guardrail

背景：

- Release convergence 继续检查 profile import evidence，因为 CSV preview/import response 的 `source` 字段会直接展示用户粘贴行的诊断摘要。
- 旧普通 source 文本过滤已覆盖 URL、路径、credential、token/secret marker 和 proxy host marker，但 `name/tags/notes/template` source 字段里的裸 IP literal 仍可能原样回显。
...
- CSV import preview 和 confirmed import 的普通 `source` 文本字段会把有效 IP literal 折叠为 `[redacted]`。
- 不改变导入行验证、profile 创建、template 解析、proxy normalization、audit metadata、runtime browser 或 proxy resolution 行为。

验证：

```bash
.venv/bin/python -m pytest backend/tests/test_bulk.py::test_profile_csv_import_responses_redact_ip_literal_source_fields -q
# RED then GREEN；旧 source.name 回显 Imported 203.0.113.88；GREEN 1 passed
...
## 2026-06-07 Automation about URL IP literal path release guardrail

背景：

- Release convergence 继续检查 Automation pages evidence，因为 about URL 会通过单独的低敏规范化路径进入 `/api/profiles/{profile_id}/automation/pages`。
- 上一轮 URL host guardrail 覆盖了普通 `http/https` URL 的 IP literal host，但 `about:203.0.113.45?token=...#...` 的 IP literal 位于 about path，旧实现清掉 query/fragment 后仍会回显 `about:203.0.113.45`。
- 当前策略下，Pixelscan `PXLSCN-FINGERPRINT-MASKING`、IPhey 以及同类底层/第三方 fingerprint 检测失败先标阻塞，不在 Manager 侧硬解；本轮只收 Manager 自己可控的 Automation pages response evidence 边界。

已覆盖：

- Automation about URL public formatter 现在会把有效 IP literal path 折叠为空 public URL。
- query、fragment 和 token marker 仍按既有逻辑清理；普通 about URL、内部页过滤、Automation capture、page refs、task execution、runtime browser 和 proxy resolution 行为保持不变。

验证：

```bash
.venv/bin/python -m pytest backend/tests/test_api.py::test_automation_pages_redacts_about_url_ip_literal_path -q
# RED then GREEN；旧 Automation pages response 回显 about:203.0.113.45；GREEN 1 passed
...
## 2026-06-07 Automation URL malformed IPv6 release guardrail

背景：

- Release convergence 继续检查 Automation URL response evidence，因为 pages、console location 和 network summary 会把运行时 URL 字符串展示给 API/debug/release triage。
- 旧 `_automation_public_about_url()` 和 `_automation_safe_url()` 直接调用 `urlparse()`；当 URL 形如 `https://[2001:db8::1/app?token=...` 时，`urllib.parse` 抛出 `ValueError`，Automation pages response 可能从低敏 evidence 读取变成接口异常。
- 当前策略下，Pixelscan `PXLSCN-FINGERPRINT-MASKING`、IPhey 以及同类底层/第三方 fingerprint 检测失败先标阻塞，不在 Manager 侧硬解；本轮只收 Manager 自己可控的 Automation URL response stability/evidence 边界。

已覆盖：

- Automation about URL formatter 和 http/https URL sanitizer 遇到 malformed IPv6 URL 会折叠为空 public URL。
- pages URL、console location URL 和 network summary URL 不回显 malformed IPv6 literal、query、fragment 或 token。
- 正常 domain URL、合法 IPv6 URL 的既有 IP literal redaction、malformed port redaction、about URL handling、Automation task execution、runtime browser、proxy resolution 和 VNC forwarding 行为保持不变。

验证：

```bash
.venv/bin/python -m pytest backend/tests/test_api.py::test_automation_url_evidence_redacts_malformed_ipv6_urls -q
# RED then GREEN；旧 Automation pages response 在 urllib.parse 解析 malformed IPv6 URL 时抛 ValueError；GREEN 1 passed

.venv/bin/python -m pytest backend/tests/test_api.py -k "automation_url_evidence or automation_pages_redacts_about_url or automation_pages_filters_internal_about or automation_pages_redacts_non_http" -q
# 9 passed, 276 deselected
```
...
## 2026-06-07 Profile bundle localStorage malformed IPv6 origin release guardrail

背景：

- Release convergence 继续检查 profile bundle localStorage export，因为该路径会把 running profile 当前 page origin 作为 bundle response/audit evidence。
- 旧 `_origin_from_page_url()` 已处理 malformed port，但 `urlparse()` 本身遇到 `https://[2001:db8::1/...` 这类 malformed IPv6 netloc 会抛 `ValueError`，profile bundle localStorage export 可能从固定低敏错误变成接口异常。
- 当前策略下，Pixelscan `PXLSCN-FINGERPRINT-MASKING`、IPhey 以及同类底层/第三方 fingerprint 检测失败先标阻塞，不在 Manager 侧硬解；本轮只收 Manager 自己可控的 profile bundle localStorage export stability/evidence 边界。

已覆盖：

- Profile bundle localStorage origin helper 遇到 malformed IPv6 URL 会返回 unsafe origin，导出接口返回固定 `Local storage origin unavailable`。
- 页面 localStorage 不会被读取，IPv6 literal、query、fragment 和 token 不进入 response/audit。
...
# RED then GREEN；旧 localStorage bundle export 在 urllib.parse 解析 malformed IPv6 URL 时抛 ValueError；GREEN 1 passed
...
- `cbim-23h.1` 的最终 all-clear 仍不能声称 Pixelscan/IPhey 全通过；本轮只收 profile bundle localStorage malformed IPv6 origin release boundary。

## 2026-06-07 Diagnostics Firefox identity summary release guardrail

背景：

- Release convergence 继续检查 `/api/diagnostics`，因为该 endpoint 会展示 Firefox/InvisiblePlaywright version、BuildID、stealth pref count/category 等 runtime evidence。
- BrowserManager 正常路径已经输出 public identity summary，但 diagnostics endpoint 旧实现直接信任该结构；如果调用边界被异常/测试污染，`stealth_pref_count` 等字段可能触发 Pydantic response 校验异常，或者让非公开 category 进入 diagnostics response。
- 当前策略下，Pixelscan `PXLSCN-FINGERPRINT-MASKING`、IPhey 以及同类底层/第三方 fingerprint 检测失败先标阻塞，不在 Manager 侧硬解；本轮只收 Manager 自己可控的 diagnostics response stability/redaction 边界。

已覆盖：

- Diagnostics endpoint 在 response 组装前会二次低敏化 Firefox identity summary。
...
# RED then GREEN；旧 diagnostics endpoint 对污染的 stealth_pref_count 抛 Pydantic ValidationError；GREEN 1 passed
...
- `cbim-23h.1` 的最终 all-clear 仍不能声称 Pixelscan/IPhey 全通过；本轮只收 diagnostics Firefox identity summary release boundary。

## 2026-06-07 VNC WebSocket origin IP literal log release guardrail

背景：

- Release convergence 继续检查 VNC/profile viewer 和 runtime viewer 的 WebSocket origin rejection 日志，因为这类 warning 会作为 Manager-controlled viewer lifecycle evidence 进入排障记录。
- 旧 `_websocket_public_host_label()` 已折叠 query/fragment 和 sensitive marker host，但 IP literal origin/host 仍可能以 `203.0.113.42` 或 `2001:db8::42` 形式写入日志。
- 当前策略下，Pixelscan `PXLSCN-FINGERPRINT-MASKING`、IPhey 以及同类底层/第三方 fingerprint 检测失败先标阻塞，不在 Manager 侧硬解；本轮只收 Manager 自己可控的 VNC/WebSocket origin log redaction 边界。

已覆盖：

- VNC WebSocket origin mismatch 日志把 IP literal origin/host 折叠为 `[redacted-ip]`。
- 非默认端口仍可作为低敏上下文保留，例如 `[redacted-ip]:8080`。
- Origin path、query、fragment、viewer_token marker、IPv4 literal 和 IPv6 literal 不进入日志。
...
# RED then GREEN；旧 VNC WebSocket origin mismatch log emitted 203.0.113.42 and 2001:db8::42；GREEN 1 passed
...
- 同类底层/第三方 fingerprint 检测站失败先标阻塞项，再继续推进 Manager 可控 API/UI/log/audit/diagnostics/runtime/proxy/profile evidence。
- 不改变 browser runtime、`invisible_playwright` 包、stealth prefs、fingerprint seed、WebGL/canvas/noise、UA、locale/timezone、WebRTC、proxy resolution、VNC forwarding、runtime session storage、viewer token schema、Automation worker lease behavior 或 browser fingerprint 行为。
- `cbim-23h.1` 的最终 all-clear 仍不能声称 Pixelscan/IPhey 全通过；本轮只收 VNC WebSocket origin IP literal log release boundary。

## 2026-06-07 VNC WebSocket IPv6 same-origin release guardrail

背景：

- Release convergence 继续检查 VNC/profile viewer 和 runtime viewer 的 WebSocket origin comparison，因为它保护 viewer session，同时不能误拒绝合法 same-origin 访问。
- 旧 origin comparison 会把 `Origin: http://[2001:db8::42]:8080` 解析成无括号 host，再和原始 `Host: [2001:db8::42]:8080` 比较，导致合法 bracketed IPv6 same-origin 被 4403 拒绝。
- 当前策略下，Pixelscan `PXLSCN-FINGERPRINT-MASKING`、IPhey 以及同类底层/第三方 fingerprint 检测失败先标阻塞，不在 Manager 侧硬解；本轮只收 Manager 自己可控的 WebSocket origin comparison stability 边界。

已覆盖：

- WebSocket Origin 和 Host 现在共用 compare netloc 规范化逻辑。
- Bracketed IPv6 same-origin 能通过 origin check，默认端口继续忽略，非默认端口继续参与比较。
- 跨源拒绝、低敏 origin/host 日志 label、profile VNC forwarding、runtime viewer token 校验、runtime session storage 和 viewer token schema 行为保持不变。

验证：

```bash
.venv/bin/python -m pytest backend/tests/test_api.py::test_ws_allows_bracketed_ipv6_same_origin -q
# RED then GREEN；旧 WebSocket origin comparison rejected bracketed IPv6 same-origin with 4403；GREEN 1 passed

.venv/bin/python -m pytest backend/tests/test_api.py::test_ws_allows_bracketed_ipv6_same_origin backend/tests/test_api.py::test_vnc_ws_rejects_cross_origin backend/tests/test_api.py::test_vnc_ws_origin_rejection_redacts_ip_literal_origin_and_host backend/tests/test_api.py::test_ws_allows_same_origin -q
# 4 passed

.venv/bin/python -m pytest backend/tests/test_api.py -k "vnc_ws_origin or ws_allows or vnc_ws_rejects_cross_origin" -q
# 7 passed, 282 deselected
```
...
- 同类底层/第三方 fingerprint 检测站失败先标阻塞项，再继续推进 Manager 可控 API/UI/log/audit/diagnostics/runtime/proxy/profile evidence。
- 不改变 browser runtime、`invisible_playwright` 包、stealth prefs、fingerprint seed、WebGL/canvas/noise、UA、locale/timezone、WebRTC、proxy resolution、VNC forwarding、runtime session storage、viewer token schema、Automation worker lease behavior 或 browser fingerprint 行为。
- `cbim-23h.1` 的最终 all-clear 仍不能声称 Pixelscan/IPhey 全通过；本轮只收 VNC WebSocket IPv6 same-origin release boundary。

## 2026-06-07 VNC WebSocket scheme-aware origin port release guardrail

背景：

- Release convergence 继续检查 VNC/profile viewer 和 runtime viewer 的 WebSocket origin comparison，因为它是 viewer session CSWSH 防线。
- 旧 compare netloc 把 `80` 和 `443` 都视为默认端口，导致 `Origin: http://testserver:443` 与 `Host: testserver` 被错误放行。
- 当前策略下，Pixelscan `PXLSCN-FINGERPRINT-MASKING`、IPhey 以及同类底层/第三方 fingerprint 检测失败先标阻塞，不在 Manager 侧硬解；本轮只收 Manager 自己可控的 WebSocket origin comparison stability 边界。

已覆盖：

- 带 scheme 的 Origin 按 scheme 判断默认端口：`http` 只忽略 `80`，`https` 只忽略 `443`。
- 无 scheme 的 Host header 继续保留既有默认端口兼容。
...
# RED then GREEN；旧 origin comparison allowed http://testserver:443 as same-origin with Host testserver；GREEN 1 passed
...
- 同类底层/第三方 fingerprint 检测站失败先标阻塞项，再继续推进 Manager 可控 API/UI/log/audit/diagnostics/runtime/proxy/profile evidence。
- 不改变 browser runtime、`invisible_playwright` 包、stealth prefs、fingerprint seed、WebGL/canvas/noise、UA、locale/timezone、WebRTC、proxy resolution、VNC forwarding、runtime session storage、viewer token schema、Automation worker lease behavior 或 browser fingerprint 行为。
- `cbim-23h.1` 的最终 all-clear 仍不能声称 Pixelscan/IPhey 全通过；本轮只收 VNC WebSocket scheme-aware origin port release boundary。

## 2026-06-07 VNC WebSocket origin log scheme port release guardrail

背景：

- Release convergence 继续检查 VNC/profile viewer 和 runtime viewer 的 WebSocket origin rejection warning，因为它是 Manager-controlled viewer lifecycle evidence。
- 行为 comparison 已经按 scheme 判断默认端口，但旧 public host label 仍把 `80` 和 `443` 都视为默认端口，导致 `http://evil.com:443` 的拒绝日志丢失 `:443` 上下文。
- 当前策略下，Pixelscan `PXLSCN-FINGERPRINT-MASKING`、IPhey 以及同类底层/第三方 fingerprint 检测失败先标阻塞，不在 Manager 侧硬解；本轮只收 Manager 自己可控的 WebSocket origin rejection log evidence 边界。

已覆盖：

- WebSocket origin rejection log label 现在按 scheme 判断默认端口。
- `http://evil.com:443` 的拒绝日志保留低敏 `origin=evil.com:443`。
- Origin path、query、fragment、viewer_token marker 仍不进入日志。
- 普通 same-origin、scheme-aware origin comparison、bracketed IPv6 same-origin、跨源拒绝、profile VNC forwarding、runtime viewer token 校验、runtime session storage 和 viewer token schema 行为保持不变。

验证：

```bash
.venv/bin/python -m pytest backend/tests/test_api.py::test_vnc_ws_origin_rejection_log_keeps_scheme_non_default_port -q
# RED then GREEN；旧 origin rejection log folded http://evil.com:443 to origin=evil.com；GREEN 1 passed

.venv/bin/python -m pytest backend/tests/test_api.py::test_vnc_ws_origin_rejection_log_keeps_scheme_non_default_port backend/tests/test_api.py::test_vnc_ws_origin_rejection_logs_low_sensitive_origin backend/tests/test_api.py::test_vnc_ws_rejects_origin_with_non_default_port_for_scheme backend/tests/test_api.py::test_vnc_ws_origin_rejection_redacts_ip_literal_origin_and_host -q
# 4 passed

.venv/bin/python -m pytest backend/tests/test_api.py -k "vnc_ws_origin or ws_allows or vnc_ws_rejects_cross_origin" -q
# 8 passed, 283 deselected
```
...
- 同类底层/第三方 fingerprint 检测站失败先标阻塞项，再继续推进 Manager 可控 API/UI/log/audit/diagnostics/runtime/proxy/profile evidence。
- 不改变 browser runtime、`invisible_playwright` 包、stealth prefs、fingerprint seed、WebGL/canvas/noise、UA、locale/timezone、WebRTC、proxy resolution、VNC forwarding、runtime session storage、viewer token schema、Automation worker lease behavior 或 browser fingerprint 行为。
- `cbim-23h.1` 的最终 all-clear 仍不能声称 Pixelscan/IPhey 全通过；本轮只收 VNC WebSocket origin log scheme port release boundary。

## 2026-06-07 Proxy redaction sensitive host marker release guardrail

背景：

- Release convergence 继续检查 Manager-controlled proxy error evidence，因为 proxy validation 错误文本会被 API/UI/log/audit/diagnostics 调试链路引用。
- 旧 `_redact_proxy_url()` 会删除 proxy credential，但敏感 marker 如果出现在 host（例如 `runtime_service_token_proxy_marker.example`），仍可能出现在 redacted proxy URL 中。
- 当前策略下，Pixelscan `PXLSCN-FINGERPRINT-MASKING`、IPhey 以及同类底层/第三方 fingerprint 检测失败先标阻塞，不在 Manager 侧硬解；本轮只收 Manager 自己可控的 proxy error redaction 边界。

已覆盖：

- `_redact_proxy_url()` 对包含 token/secret marker 的 proxy host 默认返回 `[redacted]`，保持 profile export/CSV source redaction response contract。
...
# RED then GREEN；旧 redaction returned http://runtime_service_token_proxy_marker.example:8080；GREEN 1 passed
...
## 2026-06-07 Automation task step collection release-evidence guardrail

背景：

- Release convergence 继续检查 Automation task log UI，因为 task steps、result steps 和 task detail drawer 会进入 release evidence。
- 正常 API 返回数组，但历史、mock 或异常 task JSON 可能把 `steps` 或 `result.steps` 污染成字符串、对象、`null` 或其他非数组形状。
- 旧页面在 `StepList` / `ResultList` 中直接调用 `.slice()` / `.map()`，并在 detail drawer 中直接读取 `.length`；非数组输入会让 automation evidence 页面崩溃。
- 当前策略下，Pixelscan `PXLSCN-FINGERPRINT-MASKING`、IPhey 以及同类底层/第三方 fingerprint 检测失败先标阻塞，不在 Manager 侧硬解；本轮只收 Manager 自己可控的 Automation task UI release evidence stability 边界。

已覆盖：

- Automation task step collections 现在只接受数组形状；非数组输入折叠为空列表并显示低风险 `-`。
- Result step collections 现在只接受数组形状；非数组输入折叠为空列表并显示低风险 `-`。
- Detail drawer 的 full-list `maxItems` 也使用安全 collection helper，不再读取异常 shape 的 `.length`。
- RED 确认旧页面在 `visibleSteps.map` 崩溃；GREEN 后 malformed collection 中的 token/path/IP/raw step type/raw result type 不进入 visible 或 detail evidence。
- 正常 task list、status filter、task/profile id filter、detail drawer、step summary、result summary、automation task API schema、automation execution、worker lease 和 browser fingerprint 行为保持不变。

验证：

```bash
npm --prefix frontend test -- --run src/components/AutomationTaskLogViewer.test.tsx -t "folds malformed task step collections"
# RED then GREEN；旧页面 crashed at visibleSteps.map when steps was not an array；GREEN 1 passed
...
- 这是 Manager-controlled Automation task UI release-evidence stability 防御，不是 Pixelscan/IPhey 或 `PXLSCN-FINGERPRINT-MASKING` 修复。
- 同类底层/第三方 fingerprint 检测站失败先标阻塞项，再继续推进 Manager 可控 API/UI/log/audit/diagnostics/runtime/proxy/profile evidence。
...
- `cbim-23h.1` 的最终 all-clear 仍不能声称 Pixelscan/IPhey 全通过；本轮只收 Automation task step collection release-evidence boundary。

## 2026-06-08 Automation task list response shape release-evidence guardrail

背景：

- Release convergence 继续检查 Automation task log UI，因为 task list response 会进入 stats、status filter、search filter、table 和 detail drawer release evidence。
- 正常 API 返回 `{ tasks: AutomationTask[] }`，但历史、mock 或异常 task JSON 可能把 `tasks` 污染成字符串、对象、`null` 或其他非数组形状。
- 旧页面在 stats/filter 渲染路径上直接调用 `tasks.filter(...)`；非数组输入会让 automation evidence 页面崩溃。
- 当前策略下，Pixelscan `PXLSCN-FINGERPRINT-MASKING`、IPhey 以及同类底层/第三方 fingerprint 检测失败先标阻塞，不在 Manager 侧硬解；本轮只收 Manager 自己可控的 Automation task UI release evidence stability 边界。

已覆盖：

- Automation task list response 现在只接受数组形状；非数组输入折叠为空列表并显示低风险 empty state。
- 数组内缺少 public string `id`、`profile_id` 或 `status` 的 entry 不进入 release evidence。
- RED 确认旧页面在 `tasks.filter` 崩溃；GREEN 后 malformed task list 中的 token/path/IP marker 不进入 visible 或 aria evidence。
...
# RED then GREEN；旧页面 crashed at tasks.filter when tasks was not an array；GREEN 1 passed
```
