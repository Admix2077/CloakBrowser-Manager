...
1. 继续 CloakBrowser 独立侧 07 Automation API，小步实现第一版 Script Runner executor 的剩余低风险 step，例如 `evaluate` 或 `screenshot`。这两类 step 默认不得把 expression、evaluate result 或截图内容复制进 task result/log。
