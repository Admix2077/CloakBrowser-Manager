...
## 2026-06-07 Automation URL IP literal host guardrail

背景：

- Automation pages、console location 和 network summary URL 属于 Manager-controlled Automation evidence，会把页面/请求 URL 摘要返回给 UI 和 API 调用方定位问题。
- 旧 `_automation_safe_url()` 已清理 credential、query、fragment 和敏感 marker host，但普通 IP literal host 仍可能在 page URL、console location URL 或 network URL 摘要中回显。
- 该问题只属于 Manager 自己的 Automation URL evidence 边界，不属于 Pixelscan/IPhey/PXLSCN-FINGERPRINT-MASKING 或类似底层 fingerprint detector 问题。

已覆盖：

- Automation URL sanitizer 现在遇到有效 IPv4 host 或 bracketed IPv6 host 会折叠为空，不把 IP literal 放入 pages/console location/network summary response evidence。
- 普通域名 URL 的低敏 scheme、host、port、path 摘要行为保持不变。
- 不改变 Automation capture、page refs、network capture、task execution、browser runtime、proxy resolution 或 fingerprint 行为。

验证记录：

```bash
.venv/bin/python -m pytest backend/tests/test_api.py::test_automation_url_evidence_redacts_ip_literal_hosts -q
# RED: old pages URL returned https://203.0.113.45/app
# GREEN: 1 passed
...
## 2026-06-07 Automation console IP literal redaction guardrail

背景：

- Automation console logs 属于 Manager-controlled Automation evidence，会把页面 console message text 返回给 UI 和 API 调用方定位脚本/页面问题。
- 旧 `_automation_redact_text()` 已覆盖 URL、Authorization/Bearer、Cookie、token/secret assignment 和 marker-only 文本，但裸 IPv4/IPv6 literal 仍可能原样回显。
- 该问题只属于 Manager 自己的 Automation console evidence 边界，不属于 Pixelscan/IPhey/PXLSCN-FINGERPRINT-MASKING 或类似底层 fingerprint detector 问题。

已覆盖：

- Automation console log text 现在会把有效 IPv4 literal、裸 IPv6 literal 和 bracketed IPv6 literal 折叠为 `[redacted-ip]`。
- Bracketed/endpoint 形态保留低敏端口上下文，例如 `[2001:db8::46]:443` 变为 `[redacted-ip]:443`。
- 不改变 Automation console capture、page refs、network capture、task execution、browser runtime、proxy resolution 或 fingerprint 行为。

验证记录：

```bash
.venv/bin/python -m pytest backend/tests/test_api.py::test_automation_console_logs_redacts_ip_literal_text -q
# RED: old console log text returned 203.0.113.45, 198.51.100.20, 2001:db8::45, 2001:db8::46, and 2001:db8::44
# GREEN: 1 passed
...
## 2026-06-07 Automation URL malformed IPv6 evidence guardrail

背景：

- Automation pages、console location 和 network summary response 都会通过 Manager-controlled URL sanitizer 输出低敏 URL evidence。
- 旧 `_automation_public_about_url()` 和 `_automation_safe_url()` 会直接调用 `urlparse()`；当运行时页面、console location 或 network event URL 形如 `https://[2001:db8::1/app?token=...` 这类 malformed IPv6 netloc 时，`urllib.parse` 会抛 `ValueError`，导致 response 读取变成接口异常。
- 该问题只属于 Manager 自己的 Automation URL response stability/evidence 边界，不属于 Pixelscan/IPhey/PXLSCN-FINGERPRINT-MASKING 或类似底层 fingerprint detector 问题。

已覆盖：

- Automation about URL formatter 和 http/https URL sanitizer 现在捕获 URL parse/hostname 解析异常，把 malformed URL 折叠为空 public URL。
- pages URL、console location URL 和 network summary URL 遇到 malformed IPv6 URL 时均返回低敏空 URL evidence，不回显 IPv6 literal、query、fragment 或 token。
- 正常 domain URL、合法 IPv6 URL 的既有 IP literal redaction、malformed port redaction、about URL handling、Automation task execution、runtime browser、proxy resolution、VNC forwarding 和 browser fingerprint 行为保持不变。

验证记录：

```bash
.venv/bin/python -m pytest backend/tests/test_api.py::test_automation_url_evidence_redacts_malformed_ipv6_urls -q
# RED: old Automation pages response raised ValueError from urllib.parse on malformed IPv6 URL
# GREEN: 1 passed
...
## 2026-06-07 Audit metadata malformed URL redaction guardrail

背景：

- Audit metadata sanitizer 会处理 Manager 自己持久化和读取的审计事件，并把 message 中出现的 http/https/socks5 URL 折叠成低敏 public label。
- 旧 `_public_audit_url_label()` 在 `urlsplit()` 遇到 malformed IPv6 netloc 时直接返回原 URL；后续规则会处理 token/IP，但可能留下 `username:password@` 这类 proxy credential 片段。
- 该问题只属于 Manager 自己的 audit metadata persistence/reader redaction 边界，不属于 Pixelscan/IPhey/PXLSCN-FINGERPRINT-MASKING 或类似底层 fingerprint detector 问题。

已覆盖：

- `_public_audit_url_label()` 现在遇到 URL parse 异常时，将支持的 scheme 折叠为 `scheme://unknown`。
- malformed URL 不会把 username、password、IPv6 literal、query、fragment 或 token 写入新 audit event，也不会在历史 audit event 读取时回显。
- 正常 URL label、敏感 key 删除、tuple/list metadata sanitize、audit top-level public field sanitizer、runtime browser、proxy resolution、Automation/VNC 和 browser fingerprint 行为保持不变。
...
# RED: old audit metadata sanitizer kept audit-user:audit-pass@ after urllib.parse rejected malformed IPv6 URL
# GREEN: 1 passed
...
- VNC/profile viewer 和 runtime viewer 的 WebSocket origin 校验属于 Manager-controlled viewer session protection。
- 旧 origin 比较会把 `Origin: http://[2001:db8::42]:8080` 解析成无括号 host，但直接拿原始 `Host: [2001:db8::42]:8080` 比较，导致合法 bracketed IPv6 same-origin 被误判为跨源 4403。
...
- Origin 和 Host 现在共用 `_websocket_origin_compare_netloc()` 规范化后再比较。
- IPv6 host 会以 bracketed netloc 参与比较，默认端口继续忽略，非默认端口继续保留。
- 合法 bracketed IPv6 same-origin 不再被 origin 校验拒绝。
...
# RED: old WebSocket origin comparison rejected bracketed IPv6 same-origin with 4403
# GREEN: 1 passed
...
## 2026-06-07 VNC WebSocket origin log scheme port evidence guardrail

背景：

- WebSocket origin rejection warning 是 Manager-controlled viewer lifecycle evidence，排障时需要保留低敏但准确的 host/port 上下文。
- 行为校验已经按 scheme 判断默认端口，但旧 `_websocket_public_host_label()` 仍把 `80` 和 `443` 都作为默认端口处理，导致 `http://evil.com:443` 的拒绝日志显示为 `origin=evil.com`。
- 该问题只属于 Manager 自己的 WebSocket origin rejection log evidence 边界，不属于 Pixelscan/IPhey/PXLSCN-FINGERPRINT-MASKING 或类似底层 fingerprint detector 问题。

已覆盖：

- `_websocket_public_host_label()` 现在和 comparison helper 一样按 scheme 判断默认端口。
- `http://evil.com:443` 的跨源拒绝日志保留低敏 `origin=evil.com:443`。
...
# RED: old origin rejection log folded http://evil.com:443 to origin=evil.com
# GREEN: 1 passed
...
## 2026-06-08 Runtime live verifier RFB banner evidence guardrail

背景：

- Project Mileage 远程工作台链路需要证明 CloakBrowser runtime/VNC/noVNC 真实可用。
- 旧 `backend/tests/test_runtime_live.py` 的 opt-in live verifier 已创建 runtime session、签发 viewer token，并打开 `/api/runtime/sessions/{id}/vnc` WebSocket，但只断言 WebSocket 连接成功，没有读取 VNC 后端首帧。
- WebSocket connect 只能证明代理入口可达，不能证明后端已经开始 RFB 协议握手。
