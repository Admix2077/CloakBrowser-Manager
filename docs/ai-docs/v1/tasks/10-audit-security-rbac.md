...
- [ ] 删除、导出、终止等高风险操作有确认或权限限制：
  ...
  - [x] `POST /api/proxies/{proxy_id}/assign` 后端强制要求 JSON boolean `confirm_assign: true`。
  ...
  - [x] `DELETE /api/profiles/{profile_id}/automation/pages/{page_ref}` 后端强制要求 JSON boolean `confirm_close_page: true`。
  - [x] 缺失 automation page close 确认时返回固定 `422 Automation page close requires explicit confirmation`，不调用 `page.close()`，不关闭运行中 page。
  - [x] `POST /api/proxies/bulk/check` 后端强制要求 JSON boolean `confirm_bulk_check: true`。
