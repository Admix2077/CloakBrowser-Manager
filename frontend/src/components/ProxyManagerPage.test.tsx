...
describe("ProxyManagerPage", () => {
  ...
  it("redacts assignment profile ids from rendered and search evidence", async () => {
    const leakMarker = "assignment-profile-id-secret";
    const rawProfileId =
      "assignment-id Authorization=Bearer " +
      `${leakMarker} token=${leakMarker} /data/assignment-profile-id 203.0.113.119`;
    mockListProxies.mockResolvedValue([
      proxy({ id: "proxy-1", name: "Credential Pool" }),
    ]);
    ...
    const dialog = await screen.findByRole("dialog", { name: "Assign proxy to profiles" });
    expect(within(dialog).getByText("unknown")).toBeTruthy();
    expect(within(dialog).getByText("Polluted Id Profile")).toBeTruthy();

    fireEvent.change(within(dialog).getByLabelText("Search profiles for assignment"), {
      target: { value: leakMarker },
    });
    expect(within(dialog).queryByText("Polluted Id Profile")).toBeNull();
    expect(within(dialog).getByRole("status", { name: "No assignment profiles match search" })).toBeTruthy();
  ...
  it("folds runtime and service token marker assignment profile evidence while assigning the raw id", async () => {
    const rawProfileId = "runtime_service_token-assignment-profile-id-marker";
    const rawProfileName = "service_token-assignment-profile-name-marker";
    mockListProxies.mockResolvedValue([
      proxy({ id: "proxy-1", name: "Credential Pool" }),
    ]);
    mockAssignProxyToProfiles.mockResolvedValue({
    ...
    render(<ProxyManagerPage
      profiles={[
        profile({
          id: rawProfileId,
          name: rawProfileName,
          proxy: "http://user:hiddenpass@runtime.proxy.example:8080/runtime_service_token-current-proxy-marker",
        }),
      ]}
    />);
    ...
    fireEvent.change(within(dialog).getByLabelText("Search profiles for assignment"), {
      target: { value: "runtime_service_token" },
    });
    expect(within(dialog).getByRole("status", { name: "No assignment profiles match search" })).toBeTruthy();
    ...
    fireEvent.click(within(dialog).getByLabelText("Assign unknown"));
    fireEvent.click(within(dialog).getByRole("button", { name: "Assign proxy" }));
  ...
  it("redacts selected provider preset metadata in the CSV import summary", async () => {
    const leakMarker = "preset-summary-token-super-secret";
    mockListProxies.mockResolvedValue([]);
    mockListProxyProviderPresets.mockResolvedValue([
      {
        id: "preset-polluted-summary",
        name: "Japan import default",
        provider:
          "Provider Authorization=Bearer " +
          `${leakMarker} token=${leakMarker} /data/provider-summary 203.0.113.62`,
        country_code:
          "JP Authorization=Bearer " +
          `${leakMarker} token=${leakMarker} /data/provider-country-summary 203.0.113.63`,
        tags: [{
          tag:
            "mobile Authorization=Bearer " +
            `${leakMarker} token=${leakMarker} /data/provider-tag-summary 203.0.113.64`,
          color: "#0ea5e9",
        }],
        notes: "Tokyo exits",
        created_at: "2026-05-26T00:00:00Z",
    ...
    const dialog = await screen.findByRole("dialog", { name: "Import proxy CSV" });
    fireEvent.change(within(dialog).getByLabelText("Provider preset"), {
      target: { value: "preset-polluted-summary" },
    });

    expect(within(dialog).getByText(
      "Provider [redacted] [redacted] [redacted-path] [redacted-ip] · JP [redacted] [redacted] [redacted-path] [redacted-ip] · mobile [redacted] [redacted] [redacted-path] [redacted-ip]",
    )).toBeTruthy();
    ...
    for (const leaked of [
      ...
      "/data/provider-country-summary",
      "/data/provider-tag-summary",
      "203.0.113.62",
      "203.0.113.63",
      "203.0.113.64",
    ]) {
  ...
  it("folds marker-bearing proxy CSV textarea evidence without changing the import payload", async () => {
    const existingProxy = proxy({ id: "proxy-existing", name: "Existing Pool" });
    const importedProxy = proxy({ id: "proxy-imported", name: "Imported Safe" });
    mockListProxies
      .mockResolvedValueOnce([existingProxy])
    ...
    const rawName = "api_key-proxy-csv-name-marker";
    const rawProvider = "client_secret-proxy-csv-provider-marker";
    const rawTag = "private_key-proxy-csv-tag-marker";
    const rawCsv = [
      "name,url,provider,tags",
      `"${rawName}",http://user:hiddenpass@marker.proxy.example:8080,"${rawProvider}","${rawTag}"`,
    ].join("\n");
    ...
    await waitFor(() => expect(mockCreateProxy).toHaveBeenCalledWith({
      name: rawName,
      url: "http://user:hiddenpass@marker.proxy.example:8080",
      provider: rawProvider,
      tags: [{ tag: rawTag, color: null }],
    }));
  });
